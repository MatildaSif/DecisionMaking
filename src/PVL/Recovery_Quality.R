...
load(file.path(output_dir, "PVL_recovery_results.RData"))
...
recovery_cors <- data.frame(
  ...
  Correlation = c(
    cor(true_mu_w, infer_mu_w),
    cor(true_mu_A, infer_mu_A),
...
png(file.path(output_dir, "pvl_recovery_quality_correlations.png"), width = 800, height = 500)
...
infer_sigma_w <- 1/sqrt(infer_lambda_w)
infer_sigma_A <- 1/sqrt(infer_lambda_A)
...
sigma_recovery_cors <- data.frame(
  ...
  Correlation = c(
    cor(true_lambda_w, infer_sigma_w),
    cor(true_lambda_A, infer_sigma_A),
    cor(true_lambda_theta, infer_sigma_theta),
    cor(true_lambda_a, infer_sigma_a)
  ),
  stringsAsFactors = FALSE
)
...
errors <- data.frame(
  w_error = infer_mu_w - true_mu_w,
  A_error = infer_mu_A - true_mu_A,
...
bias_cors <- data.frame(
  Parameter = c("w", "A", "theta", "a"),
  Error_vs_True = c(
    cor(errors$w_error, true_mu_w),
    cor(errors$A_error, true_mu_A),
...
cat("\nSigma Recovery:\n")
print(sigma_recovery_cors)
cat("\nAll detailed results saved to: recovery_analysis_detailed.txt\n")
cat("===============================================\n")
