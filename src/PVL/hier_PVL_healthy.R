...
cat("Fitting hierarchical PVL model to healthy controls...\n")
...
png(file.path(output_dir, "01_group_level_posteriors_healthy.png"), width=800, height=800)
par(mfrow=c(2,2))
...
plot(density(Y$mu_a), main="Group mean: a")
dev.off()
cat("Saved: 01_group_level_posteriors_healthy.png\n")

# Save group-level precision plots
png(file.path(output_dir, "02_group_level_precisions_healthy.png"), width=800, height=800)
par(mfrow=c(2,2))
...
plot(density(Y$lambda_a), main="Group precision: lambda_a")
dev.off()
cat("Saved: 02_group_level_precisions_healthy.png\n")

# Save subject-level posterior plots (Subject 1)
png(file.path(output_dir, "03_subject_1_posteriors_healthy.png"), width=800, height=800)
par(mfrow=c(2,2))
...
plot(density(Y$a[,1]), main="Subject 1: a")
dev.off()
cat("Saved: 03_subject_1_posteriors_healthy.png\n")

# Save everything to RData file
rdata_file <- file.path(output_dir, "PVL_healthy_posteriors.RData")
save(samples, x_all, X_all, ntrials_all, nsubs, subIDs,
     file = rdata_file) 
cat("\n=== Saved posteriors to", rdata_file, "===\n")

# Save summary statistics to text file
summary_file <- file.path(output_dir, "model_summary_healthy.txt")
sink(summary_file)

cat("=== FITTED PARAMETERS (Healthy Controls) ===\n")
cat("\nGroup Means (mu):\n")
...
sink()
cat("Saved: model_summary_healthy.txt\n")
