...
p1 <- ggplot(group_means_long, aes(x = Value)) +
  ...
  labs(title = "Healthy Group-Level Posterior Distributions: Means (μ)",
       x = "Parameter Value", y = "Density") +
  theme_minimal() +
  theme(strip.text = element_text(size = 12, face = "bold"))

ggsave(file.path(output_dir, "01_group_level_posteriors_healthy.png"), 
...
p2 <- ggplot(group_precisions_long, aes(x = Value)) +
  ...
  facet_wrap(~ Parameter, scales = "free", ncol = 2) +
  labs(title = "Healthy Group-Level Posterior Distributions: Precisions (λ)",
       x = "Parameter Value", y = "Density") +
  theme_minimal() +
  theme(strip.text = element_text(size = 12, face = "bold"))

ggsave(file.path(output_dir, "02_group_level_precisions_healthy.png"),
...
p3 <- ggplot(subject1_long, aes(x = Value)) +
  ...
  facet_wrap(~ Parameter, scales = "free", ncol = 2) +
  labs(title = "Healthy Subject 1: Individual Posterior Distributions",
       x = "Parameter Value", y = "Density") +
  theme_minimal() +
  theme(strip.text = element_text(size = 12, face = "bold"))

ggsave(file.path(output_dir, "03_subject_1_posteriors_healthy.png"),
